<script lang="ts">
	...
	async function saveItem() {
		...
		try {
			...
			toast.success(`${data.schema.name} updated successfully!`);
...
<svelte:head>
	<title>Edit {data.item?.title || 'Item'} - Hyperspace CMS</title>
</svelte:head>

<div class="space-y-6">
	{#if data.error || !data.schema || !data.item}
		<Card.Root>
			<Card.Content class="py-12 text-center">
				<h3 class="text-lg font-semibold">Error</h3>
				<p class="text-muted-foreground">{data.error || 'Content not found'}</p>
			</Card.Content>
		</Card.Root>
	{:else}
		<!-- Header -->
		<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
			<div class="flex items-center gap-4">
				...
				<div>
					<h2 class="text-3xl font-bold tracking-tight">Edit {data.schema.name}</h2>
